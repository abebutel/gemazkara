...
export default function App() {
  ...
  // 1. URL Parameters - Load specific booklet directly from a shared link
  useEffect(() => {
  ...
  const handleGenerate = () => {
    if (name) {
      setIsGenerated(true);
      // Update the URL so it can be shared instantly without refreshing the page
      const newUrl = `${window.location.pathname}?name=${encodeURIComponent(name)}&gender=${gender}`;
  ...
  // 2. Updated text formatting for specific headings
      ...
      {isMobile ? (
        ...
        <nav style={{ width: '280px', padding: '30px 20px', backgroundColor: theme.card, borderLeft: '1px solid #e2e8f0', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto', fontFamily: theme.uiFont, boxShadow: '-2px 0 15px rgba(0,0,0,0.03)' }}>
          ...
          <div style={{ marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #e2e8f0' }}>
            ...
            <button onClick={handlePrint} style={{ width: '100%', padding: '10px', borderRadius: '6px', border: `1px solid ${theme.primary}`, backgroundColor: 'transparent', color: theme.primary, fontSize: '16px', fontWeight: 600, cursor: 'pointer' }}>📄 הורד / הדפס PDF</button>
          </div>
        </nav>
      )}
        ...
        <nav style={{ position: 'fixed', top: '65px', left: 0, right: 0, backgroundColor: theme.card, padding: '20px', borderBottom: `3px solid ${theme.primary}`, zIndex: 999, boxShadow: '0 10px 20px rgba(0,0,0,0.1)', fontFamily: theme.uiFont }}>
          <ul style={{ listStyle: 'none', padding: 0, lineHeight: '3', margin: 0, fontSize: '1.2rem' }}>
            ...
            <li style={{ marginTop: '20px', paddingTop: '20px', borderTop: '1px solid #e2e8f0' }}>
              ...
              <button onClick={() => { setIsMenuOpen(false); handlePrint(); }} style={{ width: '100%', padding: '12px', borderRadius: '6px', border: `1px solid ${theme.primary}`, backgroundColor: 'transparent', color: theme.primary, fontSize: '16px', fontWeight: 600, cursor: 'pointer' }}>📄 הורד / הדפס PDF</button>
            </li>
          </ul>
        </nav>
      )}
